...
-- Qualora sotto-entità dovessero ereditare l'intera chiave di una entità padre,
-- nel vincolo di chiave esterna devono essere esplicitate le colonne in un singolo
-- passaggio.

CREATE TABLE Studente (
	EMAIL_STUDENTE VARCHAR(255),
    PSWD_STUDENTE VARCHAR(255),
    ANNO_IMMATRICOLAZIONE INT(4) NOT NULL,
    CODICE VARCHAR(16) NOT NULL,
    PRIMARY KEY (EMAIL_STUDENTE, PSWD_STUDENTE),
    FOREIGN KEY (EMAIL_STUDENTE, PSWD_STUDENTE) REFERENCES Utente(EMAIL, PSWD)
)ENGINE=INNODB ;

CREATE TABLE Docente (
	EMAIL_DOCENTE VARCHAR(255),
    PSWD_DOCENTE VARCHAR(255),
    NOME_DIPARTIMENTO VARCHAR(255) NOT NULL,
    NOME_CORSO VARCHAR(255) NOT NULL,
    PRIMARY KEY (EMAIL_DOCENTE, PSWD_DOCENTE),
    FOREIGN KEY (EMAIL_DOCENTE, PSWD_DOCENTE) REFERENCES Utente(EMAIL, PSWD)
)ENGINE=INNODB ;
