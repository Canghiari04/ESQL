...
DROP PROCEDURE IF EXISTS Aggiornamento_Tabella;
...
DELIMITER |
CREATE PROCEDURE Inserimento_Tabella_Esercizio(IN NOME VARCHAR(255), IN DATA_CREAZIONE DATETIME, IN NUM_RIGHE INT)
BEGIN
...
/*
DELIMITER |
CREATE PROCEDURE Aggiornamento_Tabella(IN ID_TABELLA INT, IN NOME VARCHAR(255))
BEGIN
	DECLARE countTabella INT DEFAULT 0;
	SET countTabella=(SELECT COUNT(*) FROM Tabella_Esercizio WHERE (Tabella_Esercizio.NOME=NOME));
	IF (countTabella>0) THEN 
		ALTER TABLE NOME ADD ID_TABELLA_ESERCIZIO INT DEFAULT 0;
        ALTER TABLE NOME ADD FOREIGN KEY(ID_TABELLA_ESERCIZIO) REFERENCES Tabella_Esercizio(ID) ON DELETE CASCADE;
        UPDATE NOME SET ID_TABELLA_ESERCIZIO=ID_TABELLA;
    END IF;
END;
|
DELIMITER ;    
