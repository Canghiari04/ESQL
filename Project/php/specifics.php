            ...
            <div>
                <?php 
                    ...
                    if ($_SERVER["REQUEST_METHOD"] == "POST") {
                        ...
                        $query = "SELECT Attributo.ID, Attributo.TIPO, Attributo.NOME FROM Combinazione, Tabella_Esercizio, Attributo WHERE (Combinazione.ID_TABELLA = Tabella_Esercizio.ID) AND (Combinazione.ID_ATTRIBUTO = Attributo.ID) AND (Tabella_Esercizio.ID = $spec)";
                        ...
                        if($query_run) {
                            while($row = $query_run->fetch(PDO::FETCH_OBJ))
                                echo '
                                    <div class="div-table">
                                        <form action="specifics.php" method="POST">
                                            <table>
                                                <tr>
                                                    <th> '.$row -> NOME.'</th>
                                                    <th> '.$row -> TIPO.'</th>
                                                    <th> <button type="submit" name="deleteSpecific" value='.$row -> ID.'><img class="zoom-on-img img-delete" width="16" height="16" src="img/undo.png"></button> </th>                               
                                                </tr>
                                            </table>
                                        </form>
                                    </div>
